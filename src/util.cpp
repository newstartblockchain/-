...
const char * const BITCOIN_CONF_FILENAME = "smilecoin.conf";
const char * const BITCOIN_PID_FILENAME = "smilecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "smilecoin" is a composite category enabling all smilecoin-related debug output
            if(ptrCategory->count(string("smilecoin"))) {
...
#else
    const char* pszModule = "smilecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\smilecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\smilecoinCore
    // Mac: ~/Library/Application Support/smilecoinCore
    // Unix: ~/.smilecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "smilecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/smilecoinCore";
#else
    // Unix
    return pathRet / ".smilecoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty smilecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override smilecoin.conf
